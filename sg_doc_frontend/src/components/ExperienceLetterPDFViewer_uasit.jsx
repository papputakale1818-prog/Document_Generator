...
// // ─── Build HTML ───────────────────────────────────────────────────────────────
// function buildExperienceHTML(form) {
...
//     '<meta charset="UTF-8">' +
//     '<title>Experience Letter — ' + (form.empName || 'Employee') + ' — ' + CO.fullName + '</title>' +
//     '<style>' +
...
//       /* ── content area: same top padding as Relieving Letter ── */
...
//           '<div style="font-size:13pt;font-weight:700;color:#1a1a1a">' + CO.letterPrefix + '</div>' +
...
//         /* ── ROW 3: Title "To Whom It May Concern" ── */
//         '<div style="text-align:center;font-size:15pt;font-weight:700;text-decoration:underline;margin-bottom:6mm;letter-spacing:0.02em;color:#1a1a1a">' +
//           'To Whom It May Concern' +
//         '</div>' +

//         /* ── Para 1 ── */
//         '<p style="font-size:13pt;line-height:1.7;margin-bottom:5mm;text-align:justify;color:#1a1a1a">' +
//           'This is to certify that <strong>Mr. ' + (form.empName || '[Employee Name]') + '</strong> has been employed with <strong>' + CO.fullName + '</strong>' +
//           ' from <strong>' + fmtDateShort(form.joiningDate) + ' to ' + fmtDateShort(form.relievingDate) + '</strong>' +
//           ' holding the position of <strong>' + (form.designation || '[Designation]') + '.</strong>' +
//           ' Throughout their tenure, he has exhibited exceptional professionalism, dedication, and competence in their role.' +
//         '</p>' +

//         /* ── Para 2 ── */
//         '<p style="font-size:13pt;line-height:1.7;margin-bottom:6mm;text-align:justify;color:#1a1a1a">' +
//           'During their employment, <strong>' + (form.empName || '[Employee Name]') + '</strong> demonstrated strong skill and made valuable contributions to' +
//           ' project. Their commitment to excellence, combined with their strong work contributed to the success of our' +
//           ' team and the achievement of our organizational goals.' +
//         '</p>' +
...
//             '<p style="font-size:12pt;font-weight:700;color:#1a1a1a;margin-top:4mm">HR Department</p>' +
//           '</div>' +
//         '</div>' +
...
// // ─── Hook ─────────────────────────────────────────────────────────────────────
// // form = { empName, empId, designation, joiningDate, relievingDate, letterDate }
// export function useExperienceLetterPDF(form) {
//   const exportPDF = useCallback(() => {
//     const html = buildExperienceHTML(form || {})
...
// export default useExperienceLetterPDF
  ...
  return (
    ...
    '<div class="page">' +
      ...
      '<div class="content">' +
        ...
        /* ── ROW 2: Title "To Whom It May Concern" ── */
        ...
        /* ── Para 1 ── */
        '<p style="font-size:13pt;line-height:1.7;margin-bottom:5mm;text-align:justify;color:#1a1a1a">' +
          'This is to certify that <strong>Mr /Mrs. ' + (form.empName || '[Employee Name]') + '</strong> has been employed with <strong>' + CO.fullName + '</strong>' +
          ' from <strong>' + fmtDateShort(form.joiningDate) + ' to ' + fmtDateShort(form.relievingDate) + '</strong>' +
          ' holding the position of <strong>' + (form.designation || '[Designation]') + '.</strong>' +
          ' Throughout their tenure, he has exhibited exceptional professionalism, dedication, and competence in their role.' +
        '</p>' +
    ...
    '<div id="save-bar">' +
      '<div>' +
        '<div style="color:#fff;font-size:14px;font-weight:600;margin-bottom:3px">✅ Experience Letter Ready Here! — ' + CO.fullName + '</div>' +
        '<div style="color:#8ab4d4;font-size:12px">👇 <strong style="color:#a8d4f5">Save as PDF</strong> click करा</div>' +
      '</div>' +
      '<div style="display:flex;gap:10px">' +
        '<button onclick="document.getElementById(\'save-bar\').style.display=\'none\';window.print();" style="background:linear-gradient(135deg,' + CO.color + ',#2a6bb5);color:#fff;border:none;padding:11px 28px;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer">⬇️ Save as PDF</button>' +
