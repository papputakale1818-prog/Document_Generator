...
export default function ForgotPassword() {
  ...
  const handleVerifyOtp = async () => {
    if (form.otp.length !== 6) { setError('Please enter the 6-digit OTP'); return }
  ...
  const otpInp = {
    ...
    letterSpacing: '.5em', padding: '14px 14px',
  }
  const eyeBtn = {
    position: 'absolute', right: 12, top: '50%',
    transform: 'translateY(-50%)',
  ...
  if (step === 'done') return (
    <div style={page}>
      <div style={card}>
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 52, marginBottom: 16 }}>🎉</div>
          <h2 style={{ fontSize: 20, fontWeight: 800, color: '#f1f5f9', marginBottom: 8 }}>Password Reset!</h2>
          <p style={{ fontSize: 13, color: '#94a3b8', lineHeight: 1.7, marginBottom: 24 }}>
      ...
      <div style={card}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 12, color: '#475569', margin: 0 }}>
            ...
            {step === 'reset' && 'OTP verified — choose a new password'}
          </p>
        </div>
          ...
          <>
            <div style={{ marginBottom: 16 }}>
              ...
              <input
                ...
                placeholder="your@email.com"
                value={form.email} onChange={set}
                onKeyDown={e => e.key === 'Enter' && !loading && form.email && handleSendOtp()}
              />
            </div>
            <button
              style={submitBtn(loading || !form.email)}
        ...
        {step === 'otp' && (
          <>
            <div style={{ marginBottom: 10 }}>
              ...
              <input
                style={otpInp}
                type="text" name="otp" inputMode="numeric" maxLength={6}
                placeholder="------"
                value={form.otp}
                onChange={e => setForm(p => ({ ...p, otp: e.target.value.replace(/\D/g, '').slice(0, 6) }))}
                onKeyDown={e => e.key === 'Enter' && !loading && form.otp.length === 6 && handleVerifyOtp()}
              />
            </div>

            <div style={{ textAlign: 'center', marginBottom: 16, fontSize: 12, color: '#64748b' }}>
              {timeLeft > 0
                ? <>Expires in <span style={{ color: '#f59e0b', fontWeight: 700 }}>{fmtTime(timeLeft)}</span></>
                : <span style={{ color: '#f87171' }}>OTP expired</span>}
            </div>

            <button
              style={submitBtn(loading || form.otp.length !== 6 || timeLeft <= 0)}
            ...
            <div style={{ textAlign: 'center', marginTop: 14, display: 'flex', justifyContent: 'center', gap: 16 }}>
              <button
                style={{ ...linkBtn, color: timeLeft > 0 ? '#475569' : '#818cf8', cursor: timeLeft > 0 ? 'not-allowed' : 'pointer' }}
              ...
              <button
                style={linkBtn}
                onClick={() => { clearInterval(timerRef.current); setForm(p => ({ ...p, otp: '' })); setError(''); setStep('email') }}
              >
                Change Email
              </button>
            </div>
        ...
        {step === 'reset' && (
          <>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  placeholder="Re-enter new password"
                  value={form.confirmPassword} onChange={set}
                  onKeyDown={e => e.key === 'Enter' && !isResetDisabled && handleResetPassword()}
                />
                <button style={eyeBtn} onClick={() => setShowConfirm(p => !p)} type="button">
                  {showConfirm ? '🙈' : '👁️'}
                </button>
            ...
            <button style={submitBtn(isResetDisabled)} onClick={handleResetPassword} disabled={isResetDisabled}>
