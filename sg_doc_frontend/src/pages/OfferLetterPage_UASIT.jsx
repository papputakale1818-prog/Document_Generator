...
const CO = {
  ...
  addressFooter: 'Office No. 204, Khandagale Complex, Near EON Hospital, Kharadi Bypass, Pune 411014',
  gstin:         '27ABJCS4985R1Z4',
  tan:           'PNES82511C',
  themeColor:    '#1a237e',   // UAS dark navy blue
  headerColor:   '#1a237e',
}
...
// ─── PDF Header block (reused on each page) ────────────────────────────────
function pdfHeader(bgDataUrl) {
  return `
    <div style="display:flex;justify-content:flex-end;align-items:flex-start;margin-bottom:20px;padding-bottom:14px;">
      <div style="font-size:10.5px;color:#37474f;text-align:right;line-height:1.75">${CO.address}<br>GSTIN: ${CO.gstin} | TAN: ${CO.tan}</div>
    </div>`
}

function pdfFooter() {
  return `
    <div style="margin-top:28px;padding-top:10px;border-top:1px solid #c5cae9;text-align:center;font-size:10px;color:#37474f">
      <a href="mailto:${CO.email}" style="color:${CO.themeColor}">${CO.email}</a> &nbsp;|&nbsp; ${CO.website}<br>
...
export default function OfferLetterPage_UASIT() {
  ...
  const [form, setForm] = useState({
    ...
    annualGross: '',
    pfEmp: true, pfEmr: true, esiOn: false,
    pfRate: 12, pfEmrRate: 13,
  })
  ...
  // ─── PDF Export via hidden iframe (popup-blocker safe) ───────────────────
  const exportPDF = useCallback(() => {
    // Convert background image to data URL to embed it properly in print
    const img = new Image()
    img.src = bgImage
    img.onload = () => {
      ...
      buildAndPrint(bgDataUrl)
    }
    img.onerror = () => {
      // If image fails, still generate PDF without background
      buildAndPrint('')
    }
  ...
  function buildAndPrint(bgDataUrl) {
    ...
    const pageStyle = `width:210mm;min-height:297mm;margin:0 auto;padding:20mm 20mm 16mm;font-family:'Segoe UI',Arial,sans-serif;font-size:13px;color:#1a1a2e;position:relative;${bgStyle}`
    ...
    const rHTML = salaryRows.map(({ label, m, y, type }) => {
      ...
      return `<tr style="${st}">
        <td style="padding:6px 10px;border:1px solid #9fa8da">${label}</td>
        <td style="padding:6px 10px;border:1px solid #9fa8da;text-align:right;font-family:monospace">${m!==null?fmtNum(m):"NA"}</td>
        <td style="padding:6px 10px;border:1px solid #9fa8da;text-align:right;font-family:monospace">${y!==null?fmtNum(y):"NA"}</td>
      </tr>`
    }).join("")

    const hdr = pdfHeader(bgDataUrl)
    const ftr = pdfFooter()
    ...
    <div style="page-break-before:always;${pageStyle}">
      ...
      <div style="display:flex;justify-content:space-between;margin-top:44px;padding:0 10px">
        <div style="text-align:center;min-width:160px">
          <div style="border-top:1.5px solid #37474f;padding-top:8px;font-size:12px;color:#37474f;line-height:1.7">
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .page{width:210mm;min-height:297mm;margin:0 auto;padding:20mm 20mm 16mm;${bgStyle}}
  ...
  table.info td:first-child{color:#37474f;width:45%;font-weight:600}
  ...
  .ctc-box{background:#e8eaf6;border:1px solid #c5cae9;border-radius:8px;padding:12px 16px;margin-bottom:20px}
  .ctc-label{font-size:11px;font-weight:700;color:#37474f;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px}
  .ctc-value{font-size:14px;font-weight:600;color:${CO.themeColor}}
  .footer-bar{margin-top:28px;padding-top:10px;border-top:1px solid #c5cae9;text-align:center;font-size:10px;color:#37474f}
...
<div class="page">
  ${hdr}
  <div style="text-align:right;margin-bottom:16px;color:#37474f;font-size:13px">${fmtDate(form.offerDate)}</div>
  <h1>Offer Letter</h1>
  <p><strong>Dear ${form.fullName || "[Employee Name]"},</strong></p>
  <p>With reference to your Resume and subsequent interview you had with us, we are pleased to appoint you as a "<strong>${form.designation || "[Designation]"}</strong>" in our organization on the following terms and conditions:</p>
  <table class="info">
...
${additionalTermsHTML}
    ...
    // ── Use hidden iframe so popup blockers don't interfere ──────────────
    let iframe = document.getElementById('__uas_print_frame__')
    if (!iframe) {
      iframe = document.createElement('iframe')
      iframe.id = '__uas_print_frame__'
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="max-w-2xl space-y-6">
          ...
          <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-8 shadow-xl">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><label className={lbl}>Designation</label><input name="designation" value={form.designation} onChange={handleChange} placeholder="e.g. Developer" className={inp} /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className={lbl}>Department</label><input name="department" value={form.department} onChange={handleChange} placeholder="e.g. Engineering" className={inp} /></div>
          ...
          <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-8 shadow-xl">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-4">
                ...
                <div><label className={lbl}>Appointment Date</label><input type="date" name="appointDate" value={form.appointDate} onChange={handleChange} className={inp+" [color-scheme:dark]"} /></div>
              </div>
              <div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {salaryRows.map(({ label, m, y, type }, i) => {
                    const cls = { gross:"bg-cyan-500/10 text-cyan-300 font-bold", deduct:"bg-rose-500/5 text-rose-300", "deduct-total":"bg-rose-500/10 text-rose-300 font-bold", net:"bg-amber-500/10 text-amber-300 font-bold", employer:"bg-blue-500/5 text-blue-300", na:"text-gray-600 italic" }[type] || "text-gray-300"
                    return (
              ...
              {[
                ...
                <div key={key} className="flex items-center justify-between py-3 border-b border-white/5">
                  ...
                  <button onClick={() => !disabled && setForm(prev => ({ ...prev, [key]: !prev[key] }))}
                    className={`w-11 h-6 rounded-full relative transition-all flex-shrink-0 ${form[key]&&!disabled?'bg-gradient-to-r from-blue-500 to-cyan-500':'bg-white/10'} ${disabled?'opacity-40 cursor-not-allowed':'cursor-pointer'}`}>
          ...
          <button onClick={exportPDF}
            className="w-full py-3 rounded-xl bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-800 font-semibold hover:scale-105 transition text-white text-sm shadow-lg">
            ⬇️ Generate Offer Letter (PDF) — 4 Pages
          </button>
