...
//   // Company rule: Monthly Gross > ₹21,000 असेल तर PF fixed ₹1,800 (cap) apply होईल, नाहीतर normal % calculation
...
//       setAiError('Please enter Monthly Gross first'); return
//     }
...
//                 onFound={async (emp) => {
//                   // ── Offer Letter fetch — monthly_gross ──
...
//               {[
//                 { key: 'auto',   label: '⚡ Auto Calculation', color: 'from-blue-600 to-cyan-500' },
...
//                   className="w-full py-2 rounded-lg bg-gradient-to-r from-blue-700 to-blue-500 text-white text-xs font-semibold hover:opacity-90 transition disabled:opacity-50 flex items-center justify-center gap-2">
//                   {aiLoading
...
//                     <td className="px-3 py-2 text-right text-xs text-blue-300 font-bold">{fmtNum(customMonthlyGross)}</td>
...
//               className="w-full py-3 rounded-xl bg-gradient-to-r from-blue-700 via-blue-600 to-cyan-500 font-semibold hover:scale-105 active:scale-95 transition text-white text-sm shadow-lg disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 flex items-center justify-center gap-2"
//             >
//               {saveLoading ? (
//                 <>
...
import { useState } from 'react'
import { useAuth } from '../context/AuthContext'
import { openPaySlipPDF_UASIT } from '../components/PaySlipPDFViewer_UASIT'
import EmployeeIdLookup from '../components/EmployeeIdLookup'
...
export default function PaySlip_UASIT() {
  ...
  const fetchExistingMonths = async (empId, year) => {
    ...
    try {
      ...
      const url = companyId ? `${API_URL}/payslips/?company_id=${companyId}` : `${API_URL}/payslips/`
      const res = await fetch(url, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (!res.ok) throw new Error('Could not fetch pay slip history')
      const all = await res.json()
      console.log('ALL PAYSLIPS FROM API:', all)
      console.log('LOOKING FOR → empId:', empId.trim(), '| year:', String(year))
      const months = Array.isArray(all)
        ? all
            .filter(p => {
              console.log('checking record → emp_id:', p.emp_id, '| year:', p.year)
              return p.emp_id === empId.trim() && String(p.year) === String(year)
            })
            .map(p => p.month)
        : []
      console.log('MATCHED MONTHS:', months)
      setExistingMonths(months)
    } catch (err) {
      setExistingMonthsError(`⚠️ ${err.message}`)
      setExistingMonths([])
...
All values monthly INR integers. Sum = ${Math.round(parseFloat(form.monthlyGross))}.`
  ...
  const formToPayslip = (f, calc) => ({
    ...
    month:           f.month,
    year:            f.year,
    attend_from:     f.attendFrom,
    attend_to:       f.attendTo,
    working_days:    f.workingDays,
    paid_days:       f.paidDays,
    lop:             f.lop,
    pl:              f.pl,
    basic:           calc.basic,
    da:              calc.da,
    ...
    <div className="flex min-h-screen bg-gradient-to-br from-slate-900 via-zinc-900 to-slate-800 text-white">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="max-w-2xl space-y-6">
          ...
          <div className="pb-8">
            <button
              onClick={async () => { const ok = await saveToDB(); if (ok) openPaySlipPDF_UASIT(formToPayslip(form, { basic, da, hra, conv, med, special, pfEmpM, esiEmpM, pt, loan, otherDed, tds, customMonthlyGross, netSalary })) }}
              disabled={saveLoading}
              className="w-full py-3 rounded-xl bg-gradient-to-r from-blue-700 via-blue-600 to-cyan-500 font-semibold hover:scale-105 active:scale-95 transition text-white text-sm shadow-lg disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 flex items-center justify-center gap-2"
            >
              {saveLoading ? (
