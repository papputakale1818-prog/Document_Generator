...
//       // extra safety — backend filter मिस झाला तरी इथे double-check
...
export default function OfferLetterPage_SoftGrid() {
  ...
  const [form, setForm] = useState({
    ...
    salaryInputMode: 'monthly',
    withPF:    true,
    withPFEmr: true,   // ✅ UASIT कडून: Employer PF toggle
    pfRate:    12,
    pfEmrRate: 13,
  })
  ...
  const fetchEmployeeById = async (empId) => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 404) throw new Error(`Employee ID "${empId}" does not belong to ${companyName}`)
        throw new Error('Lookup failed')
      }
      const emp = await res.json()
      if (emp.company_id && String(emp.company_id) !== String(companyId)) {
  ...
  // ─── Salary Calculations (UASIT logic) ───────────────────────────────────
  const annual      = form.salaryInputMode === 'monthly'
    ? (parseFloat(form.monthlyGrossInput) || 0) * 12
    : parseFloat(form.annualGross) || 0
  const monthly     = Math.round(annual / 12)
  ...
  const grossY      = annual
  const pfWage      = basicFinal + da
  // ✅ UASIT PF logic: flat ₹1800 if salary ≥ 25000 AND withPF ON
  const pfEmpM      = form.withPF ? (monthly >= 25000 ? 1800 : 0) : 0
  // ✅ UASIT PF Employer: 13% of 21000 ceiling if withPF + withPFEmr ON
  ...
  const letterData = {
    ...
    pf_rate:         Number(form.pfRate),
    pf_emr_rate:     Number(form.pfEmrRate),
    // ✅ UASIT कडून: withPFEmr flag PDF साठी
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="max-w-2xl space-y-6">
          ...
          <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-8 shadow-xl">
            ...
            {/* ✅ UASIT कडून: Employer PF toggle + rate inputs */}
            {form.withPF && (
              <div className="mt-5 pt-4 border-t border-white/10 space-y-4">
                <div className="flex items-center justify-between py-2">
                  ...
                  <button
                    ...
                    className={`w-11 h-6 rounded-full relative transition-all flex-shrink-0 ${form.withPFEmr ? 'bg-gradient-to-r from-indigo-500 to-purple-500' : 'bg-white/10'}`}
                  >
