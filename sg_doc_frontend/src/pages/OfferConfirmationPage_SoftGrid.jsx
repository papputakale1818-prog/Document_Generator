...
// ─── PDF Header (reused on each page) ─────────────────────────────────────
function pdfHeader() {
  return `
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px;padding-bottom:14px;border-bottom:3px solid ${CO.themeColor}">
      <div style="display:flex;align-items:center;gap:10px">
        <div style="width:44px;height:44px;background:${CO.themeColor};border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:17px">${CO.initials}</div>
        <div>
          <div style="font-size:16px;font-weight:700;color:${CO.themeColor}">${CO.fullName}</div>
          <div style="font-size:11px;color:#546e54">${CO.website}</div>
        </div>
      </div>
      <div style="font-size:10.5px;color:#546e54;text-align:right;line-height:1.75">${CO.address}<br>GSTIN: ${CO.gstin} | TAN: ${CO.tan}</div>
    </div>`
}

function pdfFooter() {
  return `
    <div style="margin-top:28px;padding-top:10px;border-top:1px solid #c8e6c9;text-align:center;font-size:10px;color:#546e54">
      <a href="mailto:${CO.email}" style="color:${CO.themeColor}">${CO.email}</a> &nbsp;|&nbsp; ${CO.website}<br>
      ${CO.addressFooter}<br>GSTIN: ${CO.gstin} | TAN: ${CO.tan}
    </div>`
}

// ─── Main Page ─────────────────────────────────────────────────────────────
export default function OfferLetterConfirmationPage_SoftGrid() {
  ...
  const exportPDF = useCallback(() => {
    ...
    const pageStyle = `width:210mm;min-height:297mm;margin:0 auto;padding:20mm 20mm 16mm;font-family:'Segoe UI',Arial,sans-serif;font-size:13px;color:#1a2e1a;${bgStyle}`

    const rHTML = salaryRows.map(({ label, m, y, type }) => {
      const st = {
        gross:          `background:#c8e6c9;font-weight:700;color:#1b5e20`,
        deduct:         `background:#fff;color:#333`,
        "deduct-total": `background:#c8e6c9;font-weight:700;color:#1b5e20`,
        net:            `background:#c8e6c9;font-weight:700;color:#1b5e20`,
        employer:       `background:#fff;color:#333`,
        na:             `color:#aaa;font-style:italic;background:#fff`,
      }[type] || `background:#fff;color:#333`
      return `<tr style="${st}">
        <td style="padding:6px 10px;border:1px solid #a5d6a7">${label}</td>
        <td style="padding:6px 10px;border:1px solid #a5d6a7;text-align:right;font-family:monospace">${m!==null?fmtNum(m):"NA"}</td>
        <td style="padding:6px 10px;border:1px solid #a5d6a7;text-align:right;font-family:monospace">${y!==null?fmtNum(y):"NA"}</td>
      </tr>`
    }).join("")
    ...
    <div style="page-break-before:always;${pageStyle}">
      ...
      <div style="display:flex;justify-content:space-between;margin-top:44px;padding:0 10px">
        <div style="text-align:center;min-width:160px">
          <div style="border-top:1.5px solid #546e54;padding-top:8px;font-size:12px;color:#546e54;line-height:1.7">
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:'Segoe UI',Arial,sans-serif;color:#1a2e1a;font-size:13px}
  ...
  table.info td:first-child{color:#546e54;width:45%;font-weight:600}
  ...
  .ctc-box{background:#e8f5e9;border:1px solid #c8e6c9;border-radius:8px;padding:12px 16px;margin-bottom:20px}
  .ctc-label{font-size:11px;font-weight:700;color:#546e54;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px}
  .ctc-value{font-size:14px;font-weight:600;color:${CO.themeColor}}
  .footer-bar{margin-top:28px;padding-top:10px;border-top:1px solid #c8e6c9;text-align:center;font-size:10px;color:#546e54}
...
<div class="page">
  ${pdfHeader()}
  <div style="text-align:right;margin-bottom:16px;color:#546e54;font-size:13px">${fmtDate(form.offerDate)}</div>
...
<div class="no-print" style="position:fixed;bottom:0;left:0;right:0;background:linear-gradient(135deg,#1e293b,#0f172a);border-top:2px solid #6366f1;padding:16px 28px;display:flex;align-items:center;justify-content:space-between;z-index:9999;font-family:'Segoe UI',sans-serif;">
  ...
  <div style="display:flex;gap:10px">
    <button onclick="this.closest('div').style.display='none';window.print()" style="background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;border:none;padding:11px 28px;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer">⬇️ Save as PDF</button>
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="max-w-2xl space-y-6">
          ...
          <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-8 shadow-xl">
            <h2 className="text-base font-semibold text-green-400 mb-5">👤 Employee Information</h2>
          ...
          <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-8 shadow-xl">
            ...
            <div className="grid grid-cols-3 gap-3 mt-5 mb-5">
              {[
                ...
                { label: "Annual CTC",   value: ctcA,    cls: "text-green-400",   bg: "bg-green-500/10 border-green-500/20" },
            ...
            <div className="rounded-xl overflow-hidden border border-white/10 mt-2">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gradient-to-r from-green-500/50 to-cyan-500/30">
                ...
                <tbody>
                  {salaryRows.map(({ label, m, y, type }, i) => {
                    const cls = {
                      ...
                      employer:       "bg-green-500/5 text-green-300",
            ...
            <div className="mt-4 bg-green-500/10 border border-green-500/20 rounded-xl px-4 py-3">
              ...
              <div className="text-green-300 font-semibold text-sm">{toWords(ctcA)} Only</div>
            </div>
          </div>
                ...
                <div key={key} className="flex items-center justify-between py-3 border-b border-white/5">
                  ...
                  <button
                    ...
                    className={`w-11 h-6 rounded-full relative transition-all flex-shrink-0
                      ${form[key] && !disabled ? 'bg-gradient-to-r from-green-500 to-cyan-500' : 'bg-white/10'}
