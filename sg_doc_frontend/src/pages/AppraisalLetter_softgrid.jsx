...
export default function SalaryAppraisalPage_SoftGrid() {
  ...
  const [form, setForm] = useState({
    ...
    withPF: true, pfRate: 12, pfEmrRate: 13,
  })
  ...
  // Called by EmployeeIdLookup after a successful fetch
  const handleEmployeeFound = async (emp) => {
    // Also fetch current monthly gross from offer_letters table
    let curMonthly = ''
    try {
      const token = localStorage.getItem('hr_token')
      const offerRes = await fetch(
        `http://127.0.0.1:8000/offer-letters/${encodeURIComponent(emp.emp_id || emp.employee_id || '')}`,
        { headers: { Authorization: `Bearer ${token}` } }
      )
      if (offerRes.ok) {
        const offerData = await offerRes.json()
        curMonthly = offerData.monthly_gross || ''
      }
    } catch {}
    ...
    setForm(prev => ({
      ...
      curMonthly:  curMonthly || emp.current_salary || emp.monthly_salary || prev.curMonthly,
    }))
  }

  const cur = calcSalary(form.curMonthly, form.withPF, form.pfRate, form.pfEmrRate)
        ...
        <div className="max-w-3xl space-y-6">
          ...
          <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-8 shadow-xl">
            ...
            <div className="mt-6 pt-5 border-t border-white/10">
              <h3 className="text-sm font-semibold text-purple-400 mb-3">🛡️ PF Option</h3>
              <div className="grid grid-cols-2 gap-4">
                <button
                  type="button"
                  onClick={() => setForm(p => ({ ...p, withPF: true }))}
                  className={`py-3 rounded-xl border-2 font-semibold text-sm transition-all ${form.withPF ? 'border-indigo-500 bg-indigo-500/20 text-indigo-300 shadow-lg shadow-indigo-500/20' : 'border-white/10 bg-white/3 text-gray-400 hover:border-white/20'}`}
                >
                ...
                <button
                  ...
                  className={`py-3 rounded-xl border-2 font-semibold text-sm transition-all ${!form.withPF ? 'border-rose-500 bg-rose-500/20 text-rose-300 shadow-lg shadow-rose-500/20' : 'border-white/10 bg-white/3 text-gray-400 hover:border-white/20'}`}
                >
                  <div className="text-xl mb-1">❌</div>
                  <div>Without PF</div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {ROWS.map((r, i) => {
                      ...
                      const naCur = isPFRow && !cur.withPF
                      const naNw  = isPFRow && !nw.withPF
                      return (
                        <tr key={i} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                          ...
                          <td className="px-4 py-2 text-right text-xs text-gray-300">
                            {naCur ? <span className="text-red-400">Not Applicable*</span> : fmtNum(cur[r.key])}
                          </td>
                          <td className="px-4 py-2 text-right text-xs text-green-400">
                            {naNw ? <span className="text-red-400">Not Applicable*</span> : fmtNum(nw[r.key])}
                          </td>
                        </tr>
                      )
                    })}
