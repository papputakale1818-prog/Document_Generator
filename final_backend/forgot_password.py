...
"""
Forgot Password module — OTP-based password reset flow (MySQL backend).

THIS FILE PROVIDES 3 ENDPOINTS matching your React frontend:
...
────────────────────────────────────────────────────────────────────
INTEGRATION STEPS (adjust the 2 import lines marked "ADJUST THIS"):
────────────────────────────────────────────────────────────────────
1. This file ASSUMES your existing project already has:
     - database.py  -> exposes `Base`, `engine`, `get_db()`
     - models.py    -> exposes a `User` class with `.email` and
                        `.hashed_password` columns
   If your column/file names differ, just edit the two import lines
   below marked "ADJUST THIS" to match your project.

2. In your main.py, add:
     from forgot_password import router as forgot_password_router
     app.include_router(forgot_password_router)

3. Create the new table (one-time):
     from database import Base, engine
     import forgot_password  # so the model is registered with Base
     Base.metadata.create_all(bind=engine)

   (If you use Alembic migrations instead, generate a migration for
   the new `password_reset_otps` table.)

4. Install any missing dependencies:
     pip install passlib[bcrypt] python-dotenv email-validator pymysql

5. Add to your .env (see .env.example provided alongside this file):
