...
# # ── Helper: get assigned company ids for a user ──────────────────────────────
# def get_company_ids(user_id: int, db: Session) -> List[int]:
#     rows = db.execute(
#         user_company_table.select().where(user_company_table.c.user_id == user_id)
#     ).fetchall()
...
#     db.execute(
#         user_company_table.delete().where(user_company_table.c.user_id == user_id)
#     )
#     # 2. insert new ones (validate each company exists)
#     for cid in company_ids:
#         company = db.query(Company).filter(Company.id == cid).first()
#         if not company:
#             raise HTTPException(status_code=404, detail=f"Company id={cid} not found")
#         db.execute(
#             user_company_table.insert().values(user_id=user_id, company_id=cid)
#         )
#     db.commit()
...
#     # company assignments CASCADE होतात (FK ondelete=CASCADE)
#     db.delete(user)
#     db.commit()
...
# # ════════════════════════════════════════════════════════════════════════════
# # COMPANIES — assign / get / update (multi-company support)
# # ════════════════════════════════════════════════════════════════════════════
...
# # ════════════════════════════════════════════════════════════════════════════
# # LEGACY endpoints (backwards compat — जुने code तोडू नये म्हणून ठेवले)
# # ════════════════════════════════════════════════════════════════════════════
...
#     return {"message": "Company assigned (legacy endpoint)"}
